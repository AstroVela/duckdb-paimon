...
def sign_bundle(arguments, contents: bytearray) -> None:
    ...
    if any(path.is_symlink() for path in (output, *output.parents)) or any(
        output.iterdir()
    ):
    ...
    if any(
        root == runner_temp or root in runner_temp.parents
        for root in (incoming, output.resolve())
    ):
        raise ValueError("private signing files must be outside the downloaded bundle")
