...
"""Validate and verify one immutable Paimon provider-wheel candidate set."""
...
PROVIDER_DISTRIBUTION = "vane-extension-paimon"
TESTPYPI_JSON_BASE = "https://test.pypi.org/pypi"
MAX_METADATA_BYTES = 1024 * 1024


class ReleaseValidationError(RuntimeError):
    """Raised when Paimon distributions violate the release contract."""
...
def validate_release(directory: Path, vane_version: str) -> str:
    """Validate a complete CPython 3.10-3.14 Paimon provider release."""
    canonical_vane_version = _canonical_version(vane_version, "Vane version")
    parsed_vane_version = Version(canonical_vane_version)
    if not parsed_vane_version.is_devrelease:
        raise ReleaseValidationError(
    ...
    if len(records) != 5 or {
        ...
        raise ReleaseValidationError(
            "release directory must contain exactly five vane-extension-paimon wheels"
        )
    if {record.interpreter for record in records} != EXPECTED_INTERPRETERS:
        raise ReleaseValidationError(
            "vane-extension-paimon must contain exactly one wheel for each "
            "CPython 3.10 through 3.14"
        )
    provider_versions = {record.version for record in records}
    if len(provider_versions) != 1:
        raise ReleaseValidationError(
            "vane-extension-paimon wheels do not share one immutable version"
        )
...
def _request_json(url: str) -> tuple[int, object | None]:
    request = urllib.request.Request(
        url, headers={"User-Agent": "vane-paimon-provider-release-validator/1"}
    )
    try:
...
def _indexed_wheel_hashes(
    ...
    for item in document["urls"]:
        if not isinstance(item, dict):
            raise ReleaseValidationError(
                f"TestPyPI returned malformed files for {distribution_name}=={version}"
            )
        ...
        if filename in actual:
            raise ReleaseValidationError(
                f"TestPyPI returned duplicate file {filename} for "
                f"{distribution_name}=={version}"
            )
...
def require_index_publishable(directory: Path, version: str) -> None:
    ...
    if status != 200:
        raise ReleaseValidationError(
            f"expected absent or reusable TestPyPI version "
            f"{PROVIDER_DISTRIBUTION}=={version}, received HTTP {status}"
        )
...
def _write_github_output(path: Path, vane_version: str, paimon_version: str) -> None:
    with path.open("a", encoding="utf-8") as output:
        output.write(f"vane_version={vane_version}\n")
        output.write(f"paimon_version={paimon_version}\n")
...
def _parse_arguments() -> argparse.Namespace:
    ...
    validate = subparsers.add_parser(
        "validate", help="validate a complete immutable Paimon release set"
    )
    ...
    verify = subparsers.add_parser(
        "verify-index", help="compare the indexed release with local wheels"
    )
...
def main() -> int:
    ...
    try:
        if arguments.command == "validate":
            paimon_version = validate_release(
                arguments.directory, arguments.vane_version
            )
            if arguments.require_testpypi_publishable:
                require_index_publishable(arguments.directory, paimon_version)
