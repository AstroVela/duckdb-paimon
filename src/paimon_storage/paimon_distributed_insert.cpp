...
static PaimonDistributedCommitEnvelope DeserializeCommitEnvelope(const string &bytes) {
	...
	if (!IsCanonicalUUID(result.operation_id) || result.table_uuid.empty() || result.table_path.empty() ||
	    ...
	    result.serialized_transport.empty() || result.serialized_messages.empty() ||
...
class ArrowSchemaReleaseGuard {
public:
	explicit ArrowSchemaReleaseGuard(ArrowSchema &schema_p) : schema(schema_p) {
	}

	~ArrowSchemaReleaseGuard() {
		if (schema.release) {
...
static PaimonDistributedTargetState LoadTargetState(ClientContext &context, PaimonCatalog &catalog,
	...
	if (!arrow_schema) {
		throw IOException("Paimon table has no Arrow schema for distributed INSERT");
	}
	ArrowSchemaReleaseGuard arrow_schema_guard(*arrow_schema);
	ArrowTableSchema arrow_table;
	ArrowTableFunction::PopulateArrowTableSchema(context, arrow_table, *arrow_schema);
	result.field_names = arrow_table.GetNames();
	result.field_types = arrow_table.GetTypes();
	if (result.field_names != declared_field_names || result.field_names.size() != result.field_types.size()) {
		throw IOException("Paimon table field names and types disagree for distributed INSERT");
	}
...
static void BestEffortAbortMessages(const PaimonDistributedInsertTransport &transport,
	...
	// This is the complete extension-owned cleanup boundary. Prepared files without a materialized CommitMessage,
	// and files retained after an uncertain catalog outcome, are left for Paimon's orphan-files garbage collection.
	try {
...
static void DecodeCommitResults(const DistributedExtensionWriteInfo &info,
	...
	for (const auto &result : results) {
		...
		if (envelope.operation_id != transport.operation_id || envelope.table_uuid != transport.table_uuid ||
		    envelope.table_path != transport.table_path || envelope.schema_fingerprint != expected_schema_fingerprint ||
